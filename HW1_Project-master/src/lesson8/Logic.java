...
public class Logic {
    ...
    static char[][] map;
    static int[][] winLineArray;

    static Random random = new Random();

    static boolean isFinishedGame;
    static boolean isAiWon;
    static boolean isHumWon;
    static boolean isNotWinner;
...
// Сходить в произвольную не занятую ячейку
    ...
    static boolean checkLine(int cy, int cx, int vy, int vx, char dot, int dotsToWin) {
        ...
        winLineArray = new int[DOTS_TO_WIN][2];
        for (int i = 0; i < dotsToWin; i++) {
            if (map[cy + i * vy][cx + i * vx] != dot) {
                return false;
            }

            // заполнение координат победной линии
            winLineArray[i][0] = cx + i * vx;
            winLineArray[i][1] = cy + i * vy;
        }
        return true;
    }
